...
export default function PublicProfilePage() {
    ...
    const userProfileRef = useMemoFirebase(
        () => (userId && firestore ? doc(firestore, 'userProfiles', userId) : null),
        [userId, firestore]
    );
    ...
    const transactionsQuery = useMemoFirebase(() => {
        ...
        return query(
            ...
            or(where('fulfillerId', '==', userId), where('requesterId', '==', userId))
        );
    }, [userId, firestore]);
    ...
    const stats = useMemo(() => {
        ...
        const lent = transactions.filter(tx => tx.status === 'COMPLETED' && tx.fulfillerId === userId).length;
        const borrowed = transactions.filter(tx => tx.status === 'COMPLETED' && tx.requesterId === userId).length;
        const activeStatuses = ['CREATED', 'HANDOVER_PENDING', 'ACTIVE', 'RETURN_PENDING'];
        const active = transactions.filter(
            (tx) =>
              (tx.fulfillerId === userId || tx.requesterId === userId) &&
    ...
    if (isLoadingProfile || isLoadingTransactions || !userProfile) {
        return (
        ...
        <div className="flex flex-col gap-8">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                             <Avatar className="h-20 w-20 border-2 border-primary">
                                <AvatarImage src={userProfile.photoURL || ''} alt={displayName} data-ai-hint="person avatar"/>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>What other users are saying about their transactions with this user.</CardDescription>
                </CardHeader>
                <CardContent>
                    {isLoadingFeedbacks ? (
